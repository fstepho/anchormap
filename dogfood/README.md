...
`scan_strict.json` is the curated dogfood reference scan.
...
- The strict scan is a focused traceability signal, not a claim of full
  repository coverage.
- `scan_strict.pretty.json` is generated from the same data for review only.
- `analysis.md` records the human interpretation of what the reference scans
  teach about this repository.

## Exploratory scan

`scan_docs_exploratory.json` scans broad `docs/` anchors with an empty temporary
mapping.
...
`scan_docs_exploratory.pretty.json` is generated from the same data for review
only.
