...
`Operation not permitted`, ou impossibilité explicite d'inspecter le diff), la
review est invalide même avec exit 0. La relance in-session autorisée est la
même commande native `codex review`, via une approval ou une règle de préfixe
ciblée qui exécute cette commande hors du sandbox du coordinateur tout en
conservant le sandbox normal de la fresh review. Cette relance ne change pas la
surface reviewée, ne remplace pas la fresh review Codex, n'autorise pas un
second moteur de review, et ne peut être utilisée que si le diff reste borné à
la tâche ou à la maintenance process reviewée. Si cette relance est refusée ou
ne produit pas un verdict natif exploitable, la boucle s'arrête avec une
classification `tooling problem`.
...
router les approvals éligibles vers l'auto-reviewer, avec les permissions de
démarrage opératoires résumées dans `docs/agent-loop.md` ou un mode équivalent.
Le mode ne doit pas dépendre d'approbations humaines répétées pour
`codex review`, `git add` ou `git commit`.
