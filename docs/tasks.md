...
- Next executable product task: `T21.2 — Document PR report, policy examples, and Action troubleshooting`
- Last completed task: `T21.1 — Document minimal GitHub Action setup and inert workflow examples`
...
Selected strategy:
- Keep the `ADR-0011` release launcher profile unchanged.
...
- Supported-platform GitHub Actions run `25074907240` on `main` at `82aa83cef72c996990a9bdfdb59b3ccfe24bb416` failed Gate F after this follow-up strategy: macOS arm64 passed (`small` p95 `259.707 ms`, peak RSS `116.516 MiB`; `medium` p95 `668.841 ms`, peak RSS `120.391 MiB`), while Linux x64 failed only on `small` p95 (`420.055 ms` against the `400 ms` budget; peak RSS `89.887 MiB`). Linux x64 `medium` passed (`907.865 ms`, peak RSS `93.156 MiB`).
- Second follow-up strategy after run `25074907240`: keep the TypeScript parser profile and `ADR-0011` launcher unchanged while reducing fixed per-file work by parsing without parent pointers, collecting supported static imports/exports from top-level statements before recursing for unsupported call syntax, adding a safe-string fast path to the custom JSON renderer, and avoiding a duplicate `scan --json` validation allocation.
- Local macOS arm64 Gate F evidence for the second follow-up strategy passed from `/tmp/anchormap-t9.9-parentless-full`: `small` p95 `186.729 ms`, `small` peak RSS `117.609 MiB`, `medium` p95 `363.626 ms`, `medium` peak RSS `120.688 MiB`.
- Supported-platform GitHub Actions run `25076077820` on `main` at `73be140c5b2399d6b7c86980b2cd0b95966aff82` failed Gate F after the second follow-up strategy: Linux x86_64 failed only on `small` p95 (`403.237 ms` against the `400 ms` budget; peak RSS `89.91 MiB`) while `medium` passed (`750.22 ms`, peak RSS `93.082 MiB`); macOS arm64 failed only on `small` p95 (`409.646 ms` against the `400 ms` budget; peak RSS `115.672 MiB`) while `medium` passed (`813.093 ms`, peak RSS `118.75 MiB`).
- Third follow-up strategy after run `25076077820`: keep the TypeScript parser profile and `ADR-0011` launcher unchanged while eliminating redundant supported-target filesystem existence checks for already discovered product files, reusing static-edge candidate definition arrays, avoiding temporary existing-candidate arrays, and adding a same-directory import-candidate normalization fast path for the benchmark's dominant `./x` imports.
- Local macOS arm64 Gate F evidence for the third follow-up strategy passed from `/tmp/anchormap-t9.9-candidate-fastpath-final-full`: `small` p95 `188.974 ms`, `small` peak RSS `117.609 MiB`, `medium` p95 `360.539 ms`, `medium` peak RSS `122.188 MiB`.
...
Purpose:
- Add `anchormap scaffold --output <path>` to generate a create-only Markdown
  spec draft from observed TypeScript exports.
- Reduce startup cost without creating mappings, inferring product intent, or
  modifying `anchormap.yaml`.

Contract refs:
- `contract.md` — §4.2 Trust boundaries
- `contract.md` — §9 Command surface and scaffold command
- `contract.md` — §12.1 Root/config behavior
- `contract.md` — §12.2 User path normalization
...
Implementation scope:
- Extend the project-owned CLI parser and dispatcher with `scaffold`.
- Require exactly one `--output` option and normalize it as `RepoPath`.
- Load and validate `anchormap.yaml`; use its `product_root`, `spec_roots`, and
  `ignore_roots`.
- Validate that `--output` is under `spec_roots`, has an existing directory
  parent, and does not already exist.
- Build the current `SpecIndex` before generation and reject generated anchors
  already present in specs.
- Discover, read, decode, and parse supported `product_files` using the existing
  TypeScript profile.
- Extract only supported top-level exported declarations.
- Generate deterministic `DOTTED_ID` anchors from module path plus export name.
- Render exact Markdown and write the output file create-only with cleanup on
  failure.
- Update packaging file allowlist for the new runtime module.
- Add B-scaffold fixtures and focused unit tests.

Out of scope:
- JSDoc anchors.
- Mapping suggestions or writes to `mappings`.
- `--replace` or update-in-place behavior.
- New parser dependencies, Git, network, cache, or clock input.

Done when:
- `fx77`–`fx85` pass.
- Existing B-cli command surface fixtures still pass.
- Unit tests cover parser, extraction, ID generation, Markdown rendering, and
  write preconditions.
- `anchormap.yaml` is unchanged for every scaffold fixture.

Suggested verification:
- Run scaffold and CLI unit tests.
...
Purpose:
- Allow `anchormap scaffold` to complete on existing TypeScript repositories
  where multiple public exports normalize to the same mechanical anchor.
- Preserve deterministic generation and the Human/spec trust boundary.
...
- Keep anchor generation based on module path plus export name.
- Replace internal generated-anchor collision failure with deterministic
  suffixing by export kind only when a collision occurs.
- Apply stable ordinal suffixes when multiple candidates of the same kind still
  collide.
- Continue rejecting any generated base anchor or final anchor already observed
  in current specs.
- Update B-scaffold fixtures and unit tests for collision disambiguation.

Out of scope:
- User-configurable naming policy.
- Mapping suggestions or writes to `mappings`.
- Renaming anchors based on semantics, comments, JSDoc, or TypeScript symbol
  resolution.
- Allowing scaffold to bypass collisions with human-authored specs.

Done when:
- `fx81` verifies successful same-kind disambiguation with an exact Markdown
  golden.
- A B-scaffold fixture verifies base-anchor collision with current specs remains
  a code `4` no-mutation failure.
- A B-scaffold fixture verifies residual final-anchor collision remains a code
  `4` no-mutation failure.
- Unit tests cover mixed-kind and same-kind scaffold collisions.
- A sandboxed dogfood run verifies `scaffold` followed by `scan --json` on this
  repo's `src` product root.
