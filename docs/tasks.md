...
- Blocked tasks:
  - `T9.9 — Resolve Gate F release performance blocker`: blocked on supported-platform Gate F evidence from GitHub Actions run `25074907240` on `main` at `82aa83cef72c996990a9bdfdb59b3ccfe24bb416`; macOS arm64 passed, but Linux x86_64 failed because `small` p95 was `420.055 ms` against the `400 ms` budget.
