...
- Blocked tasks:
  - `T9.8 — Reconcile archived M9 release-gate evidence`: blocked on Linux x86_64 Gate F performance evidence; `npm run release:gates` previously passed Gates A, B, C, D, E, G and the M9 checklist, but the latest supported-platform Gate F report still fails because Linux x86_64 `small` p95 is `420.055 ms` against the `400 ms` budget.
...
- Open deviations:
  - `T9.9`: `tooling problem`, blocking for release-candidate closure; the follow-up bounded runtime optimization for repeated canonical sorting, scan reachability work, and static-edge candidate generation passed local macOS arm64 Gate F evidence but failed supported-platform GitHub Actions run `25074907240` on `main` at `82aa83cef72c996990a9bdfdb59b3ccfe24bb416`. Both supported-platform jobs reached the Gate F benchmark step and produced evaluable reports. macOS arm64 passed (`small` p95 `259.707 ms`, peak RSS `116.516 MiB`; `medium` p95 `668.841 ms`, peak RSS `120.391 MiB`), while Linux x86_64 failed only on `small` p95 (`420.055 ms` against the `400 ms` budget; peak RSS `89.887 MiB`). Linux x86_64 `medium` passed (`907.865 ms`, peak RSS `93.156 MiB`). Current operator action: validate the bounded parentless TypeScript parse / top-level static-edge extraction / JSON fast-path strategy on the supported-platform GitHub Actions workflow; do not change Gate F budgets, corpus shape, supported platforms, parser profile, release launcher profile, or benchmark pass/fail rules.
  - `T9.8`: `tooling problem`, blocking for release-candidate closure; GitHub Actions run `25074907240` on `main` at `82aa83cef72c996990a9bdfdb59b3ccfe24bb416` provides fresh supported-platform Gate F evidence, but Linux x86_64 still fails on `small` p95 `420.055 ms` against the `400 ms` budget. Next operator action: resolve or explicitly reclassify the remaining T9.9 Linux `small` performance blocker before release evidence can be reconciled.
