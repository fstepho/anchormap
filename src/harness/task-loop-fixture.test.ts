...
const SCRIPT_PATH = resolve(REPO_ROOT, "scripts", "task-loop.sh");
const FAMILY_DIR = resolve(REPO_ROOT, "fixtures", "task-loop");

function loadManifest(dir: string): TaskLoopFixtureManifest {
	const raw = readFileSync(resolve(dir, "manifest.json"), "utf8");
	const parsed = JSON.parse(raw) as TaskLoopFixtureManifest;
	return parsed;
}
...
for (const fixtureId of discoverFixtures()) {
	...
	test(`task-loop fixture: ${fixtureId}`, () => {
