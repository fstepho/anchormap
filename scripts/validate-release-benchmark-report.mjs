...
function readExpectedCorpora() {
	...
	for (const corpus of manifest.corpora) {
		...
		for (const field of ["product_files", "observed_anchors", "mapped_anchors", "supported_edges"]) {
...
function validateReport(reportPath, expected, requiredPlatformKey) {
	...
	for (const corpusId of expectedCorpusIds) {
		...
		if (!Array.isArray(result.measured_runs) || result.measured_runs.length !== expectedMeasuredRuns) {
...
function validateSupportedPlatformArtifacts(artifactRoot, expected) {
	...
	for (const platform of supportedPlatformArtifacts) {
		...
		if (!existsSync(reportPath)) {
			fail(`benchmark-report: missing supported-platform artifact ${platform.reportPath.join("/")}`)
		}
		const platformKey = validateReport(reportPath, expected, platform.key)
...
if (options.mode === "supported-platform-artifacts") {
	...
	validateReport(options.reportPath, expected, options.requiredPlatformKey)
}
