...
hard stops. Effective autopilot requires the startup permissions summarized in
`docs/agent-loop.md` so recurring `codex review`, `git add`, and `git commit`
approvals are not human gates.
